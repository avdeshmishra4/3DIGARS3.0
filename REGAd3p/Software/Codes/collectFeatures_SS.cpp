...
int main(int argc, char *argv[])
{
	...
	while (res_pos < seqLength)
	{
		...
		//char *pssm_val = substring(pssmline, 9, 63);

		int pssm_count = 1;
		int tpssm_count = 1;
		char *tpssm;
		tpssm = strtok(pssmline, " ");
		while (tpssm != NULL)
		{
			if ((tpssm_count >= 3) && (tpssm_count <= 22))
			{
				pssm_value = (double)atoi(tpssm) / 9.0;						// Collect each pssm (normalized)
				sprintf(pssm_val_str, "%.3lf", pssm_value);
		...
		{
			printf("pssmline: %s\n", pssmline);
			printf("pssm_count: %d---20 PSSM not found!!\n", pssm_count);
			exit(1);
		}
		//====================================================================================================================================
